...
import { ReportGlobalMetrics } from '@/services/reports-service';
import {
  TrendingUp,
  ArrowDownRight,
  ArrowUpRight,
...
export function ReportSummaryCards({ metrics }: ReportSummaryCardsProps) {
  ...
  const isPositiveResult = metrics.financialResult >= 0;
    ...
    <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-5 gap-4">
      ...
      <div className="p-5 rounded-2xl border border-l-4 border-l-[#235347] dark:border-l-[#377d6c] bg-[#ffffff] dark:bg-[#1e1b18] border-[#e5dfd3] dark:border-[#38322c] shadow-xs">
        <div className="flex items-center justify-between">
          <div>
            ...
            <h3 className="mt-1 text-2xl font-bold text-[#235347] dark:text-emerald-400">
              {formatCurrency(metrics.grossBilling)}
            </h3>
          </div>
        ...
        <p className="mt-3 text-xs text-[#8c7f74] dark:text-[#8a7f75]">
          Total bruto faturado em catálogo
        </p>
      </div>

      {/* Despesas / Saídas Pagas */}
      <div className="p-5 rounded-2xl border border-l-4 border-l-rose-700 dark:border-l-rose-500 bg-[#ffffff] dark:bg-[#1e1b18] border-[#e5dfd3] dark:border-[#38322c] shadow-xs">
        <div className="flex items-center justify-between">
          <div>
            <p className="text-xs font-medium tracking-wide uppercase text-[#8c7f74] dark:text-[#8a7f75]">
              Despesas Liquidadas
            ...
            <h3 className="mt-1 text-2xl font-bold text-rose-700 dark:text-rose-400">
              {formatCurrency(metrics.financialOutflows)}
            </h3>
          </div>
        ...
        <p className="mt-3 text-xs text-[#8c7f74] dark:text-[#8a7f75]">
          Saídas reais de caixa e insumos
        </p>
      </div>
      ...
      >
        <div className="flex items-center justify-between">
          <div>
            ...
            >
              {formatCurrency(metrics.financialResult)}
            </h3>
          </div>
      ...
      <div className="p-5 rounded-2xl border border-l-4 border-l-[#2c4a6f] dark:border-l-[#446d9b] bg-[#ffffff] dark:bg-[#1e1b18] border-[#e5dfd3] dark:border-[#38322c] shadow-xs">
        ...
        <p className="mt-3 text-xs text-[#8c7f74] dark:text-[#8a7f75]">
          Direitos com clientes no prazo
        </p>
      </div>
